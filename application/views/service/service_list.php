<div id="service_row_controller" class="justify-content-center mx-4 my-4">
    <div>
    ...
    <?php $this->load->view("/service/service_add");?>
    <div>
        <paginator v-bind:total_pages="pages" v-bind:current_page="current_page"></paginator>

        <table class="table table-bordered" v-if="service_list.length>0">
            <thead>
            <tr>
                <th>#</th>
                <th>Время</th>
            ...
            <tbody>
            <tr v-for="(service_row,index) in service_list">
                <td>{{service_row.id}}</td>
                <td>{{service_row.stamp}}</td>
                <td>{{service_row.flat_name}} ({{warning_row.flat_id}})</td>
                <td>{{service_row.person_name}}</td>
                <td>{{service_row.phone}}</td>
                <td>{{service_row.message}}</td>
                <td>{{service_row.pickup}}</td>
                <td>{{service_row.delivery}}</td>
                <td>
                    <span class="fa fa-pencil edit-user" v-on:click="edit_row(index)"></span>
                    <span class="fa fa-remove delete float-right" v-on:click="delete_row(index,service_row.id)"></span>
                </td>
            </tr>
            </tbody>
        </table>
    ...
    el = new Vue({
        ...
        methods: {
            search: function (page = 0) {
                ...
                axios.post("/service/search/" + page, {
                    person_id: el._data.person_id,
                    flat_id : el._data.flat_id
                }).then(function (result) {
                    switch (result.data.status) {
                        case 200:
                            el._data.service_list.splice()
                            el._data.pages.splice(0);
                            el._data.service_list = result.data.content;
            ...
            delete_row: function (index, id) {
                if (window.confirm("Вы подтверждаете удаление?")) {
                    axios.post("/service/delete/" + id, {
                        id: id,
                    }).then(function (result) {
                        switch (result.data.status) {
                            case 200:
                                el._data.service_list.splice(index, 1)
                                break;
                            case 300:
            ...
            edit_row: function (index) {
                this.new_row = el.$data.service_list[index]
            ...
            add_new_row: function (new_row) {
                ...
                var url = "/services/add_new_serv";
                if (this.new_row.edit_id != 0) {
                    url = "/services/edit_serv/" + this.new_row.edit_id;
                }
